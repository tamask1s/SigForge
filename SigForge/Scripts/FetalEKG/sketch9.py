...
{
    Copy(in_sigtmp, ekgsignal);
    gauss_1d(FKernel, 480, SAMPR, 5, -20, 3);
    gauss_1d(FKernelD, 480, SAMPR, 5, 20, 3);
    IProd(FKernelD, -1);
    ...
    RefineSpikesWithCorr(in_sigtmp, FKernel, spikes, 0.1, 0.2);
...
def SpikeDetecto2(ekgsignal, spikes, spike_radius, density_threshold, bo_low, bo_high, details)
{
    ...
    FilterReverse(ekgsignal, bandpass_filter);
    Copy(ekgsignaltmp, ekgsignal);
    gauss_1d(FKernel, 1480, 2000, 5, -7, 2.0);
    gauss_1d(FKernelD, 1480, 2000, 5, 7, 2.0);
    IProd(FKernelD, -1);
    Add(FKernel, FKernelD);
    ...
    Copy(correlated2, correlated);
    Copy(correlated3, correlated);

    //CreateFilter(bandpass_filter3, butterworth bp, 1 2000, 2.1 2.5);
    CreateFilter(bandpass_filter3, butterworth bp, 1 2000, 0.3 3.1);
    Filter(correlated2, bandpass_filter3);
    FilterReverse(correlated2, bandpass_filter3);
    Copy(correlated, correlated2);
    
    //DownsampleToNrSamples(correlated, correlated_ds, 8192);
    Downsample(correlated, correlated_ds, 50);
    Truncate(correlated_ds, 5, 5);
    MeanSpectrum(correlated_ds, spectra, 20, 100);
    Truncate(spectra, 0, 16.37);
    
    //CreateFromTo(ramp, 100, 10, -0.0001, 1000);
    //CreateFromTo(ramp, 40000, 100, -1, 30);
    //DotProdInplace(spectra, ramp);
    //DotProdInplace(spectra, ramp);
    //DotProdInplace(spectra, ramp);
    //DotProdInplace(spectra, ramp);
    //DisplayData(ramp);
    
    //MaxVal(spectramax, spectra);
    FirstSpikeFromEnd(spectramax, spectra);
    
    Copy(spectramax_border_up, spectramax);
    Copy(spectramax_border_down, spectramax);
    IAdd(spectramax_border_up, 0.4);
    IAdd(spectramax_border_down, -0.0);
    
    //CreateFilter(bandpass_filter4, butterworth bp, 1 2000, GetVal(spectramax_border_down, 0, 0) GetVal(spectramax_border_up, 0, 0));
    CreateFilter(bandpass_filter4, butterworth bp, 1 2000, 2.3 3.1);
    //CreateFilter(bandpass_filter4, butterworth bp, 1 2000, 2.1 2.5);
    //CreateFilter(bandpass_filter4, butterworth bp, 1 2000, 2.45 2.49);
    Filter(correlated3, bandpass_filter4);
    FilterReverse(correlated3, bandpass_filter4);
    Copy(correlated, correlated3);
    
    DisplayData(spectra, fit_width,, B, 001);
    DisplayData(spectramax, fit_width, value_list);
    DisplayData(correlated_ds,,, 000);
...
def FetalEkg(a_fie, a_channels_to_import, a_channel_indx_maternal, a_channel_index_fetal, a_outfile, details)
{
    ...
    Truncate(mother_sig2, 5, 5);
    SaveVarToFile(mother_sig2, a_outfile, 100, 8);
};
