...
DFileType DllFileType = {1, "*.edf;*.edfp", "European Data Format"};
...
extern "C"
{
    ...
    DllExport bool DOpenFile(int aData, char* FileName, const bool Write = true)
    {
        ...
        strtrn(TempString, Buffer + NRCHANNELS, sizeof(EDFMainHeader.NrChannels));
        Channels = atoi(TempString);
        /* Exit if no channels are specified (possible corrupt file) */
        if (!Channels)
            goto Error;
        /* Import the duration of a record */
        strtrn(TempString, Buffer + DURATION, sizeof(EDFMainHeader.Duration));
        Duration = atof(TempString);
        /* Exit if no duration is specified (possible corrupt file) */
        if (Duration == 0.0)
            goto Error;
        /* Calculate the effective header size */
        strtrn(TempString, Buffer + NRBYTES, sizeof(EDFMainHeader.NrBytes));
        NrBytes = atoi(TempString);
        /* Check for a valid EDF file by comparing the calculated and saved sizes */
...
Error:		/* Close the opened file and exit */
        ...
        /* Assuming the EDF file is a valid and correct file */
        ...
        strtrn(Data->Patient, Buffer + PATIENT, sizeof(EDFMainHeader.Patient));
        /* Import m_recording identification */
        strtrn(Data->m_recording, Buffer + RECORDING, sizeof(EDFMainHeader.m_recording));
        /* Import Start date */
        strtrn(Data->Date, Buffer + STARTDATE, sizeof(EDFMainHeader.StartDate));
        /* Import Start time */
        strtrn(Data->Time, Buffer + STARTTIME, sizeof(EDFMainHeader.StartTime));
...
Cleanup:
        ...
        unsigned int Size = sizeof(EDFChannelHeader.Label);
        char* Offset = ReadBuffer + LABEL * Channels;
        char tmp[sizeof(EDFChannelHeader.Label)];
        for (unsigned int i = 0; i < Channels; i++)
        {
            memcpy(tmp, Offset, sizeof(EDFChannelHeader.Label));
            memcpy(Data->m_labels[i], tmp, Size);
            Data->m_labels[i][Size - 1] = 0;
            Offset += Size;
        }
        /* Import the channel Transducers */
        Size = sizeof(EDFChannelHeader.Transducer);
        ...
        Size = sizeof(EDFChannelHeader.Dimension);
        ...
        Size = sizeof(EDFChannelHeader.PhysicalMin);
        ...
        Size = sizeof(EDFChannelHeader.PhysicalMax);
        ...
        Size = sizeof(EDFChannelHeader.DigitalMin);
        ...
        Size = sizeof(EDFChannelHeader.DigitalMax);
        ...
        for (unsigned int i = 0; i < Channels; i++)
        {
            ...
            if ((Data->DigitalMax[i] = atoi(TempString)) < Data->DigitalMin[i])
                Success = false;
            /* Calculate the Ratio for EDF transformation */
            Data->Ratio[i] = 0.0;
        ...
        Size = sizeof(EDFChannelHeader.Prefiltering);
        ...
        Size = sizeof(EDFChannelHeader.NrSamples);
    ...
    * necessary memory alocations will be performed.
    ...
    DllExport bool DCreateNewFile(int aData, const char* FileName, int Channels, const bool Rewrite = false)
    {
        ...
        if (Channels)
        {
            ...
            for (int i = 0; i < Channels; i++)
            {
                ...
                Data->DigitalMin[i] = -32768;
                Data->DigitalMax[i] = 32767;
                Data->JumpTable[i] = 0;
                Data->Ratio[i] = 0.0;
            }
        }
    ...
    DllExport bool DGetDataRaw(int aData, double** Buffer, unsigned int* Start, unsigned int* Stop)
    {
        ...
        short int* FileBuffer = (short int*) Data->FileBuffer;

        if (FILEBUFFER != Data->FILEBUFFER)
        {
            delete[] Data->FileBuffer;
            FileBuffer = new short int[FILEBUFFER];
        ...
        NrBlock -= StartBlock;
        unsigned int StartOffset = StartBlock * Data->RecordSize * sizeof(short int) + Data->Offset;
        DWORD NrBytes;
        ...
        for (unsigned int i = 0; i <= NrBlock; i++)
        {
            //printf("i=%u ", i);
            TempBlock = 0;
            if (!ReadFile(Data->FileHandle, FileBuffer, FILEBUFFER * sizeof(short int), &NrBytes, NULL))
                return false;
            if ((FILEBUFFER * sizeof(short int)) != NrBytes)
            ...
            for (unsigned int j = 0; j < Channels; j++)
            {
                ...
                if (p < q)
                {
                    ...
                    for (unsigned int k = p; k < q; k++)
                        Buffer[j][m + k - p] = Ratio[j] * (FileBuffer[TempBlock + k] - Data->DigitalMin[j]) + PhysicalMin[j];
                    Jump[j] += q - p;
                }
                TempBlock += r;
            }
            ...
            for (unsigned int j = 0; j < Channels; j++)
            {
                ...
                if (Enable[j])
                {
                    ...
                    if (p < q)
                    {
                        m = Jump[j];
                        for (unsigned int k = p; k < q; k++)
                            Buffer[l][m + k - p] = Ratio[j] * (FileBuffer[TempBlock + k] - \
                                                               Data->DigitalMin[j]) + PhysicalMin[j];
                        Jump[j] += q - p;
                    }
                    l++;
                }
    ...
    DllExport bool DWriteHeader(int aData)
    {
        ...
        strapd(Buffer + VERSION, "0", sizeof(EDFMainHeader.Version));
        strapd(Buffer + PATIENT, Data->Patient, sizeof(EDFMainHeader.Patient));
        strapd(Buffer + RECORDING, Data->m_recording, sizeof(EDFMainHeader.m_recording));
        strapd(Buffer + STARTDATE, Data->Date, sizeof(EDFMainHeader.StartDate));
        strapd(Buffer + STARTTIME, Data->Time, sizeof(EDFMainHeader.StartTime));
        sprintf(TempString, "%u", Length + HEADBUFFER);
        strapd(Buffer + NRBYTES, TempString, sizeof(EDFMainHeader.NrBytes));
        FillMemory(Buffer + RESERVED, sizeof(EDFMainHeader.m_reserved), ' ');
        strcpy((char*)(Buffer + RESERVED + 6), Data->m_horizontal_units);
        sprintf(TempString, "%u", Data->NrRecords);
        strapd(Buffer + NRRECORDS, TempString, sizeof(EDFMainHeader.NrRecords));
        sprintf(TempString, "%f", Data->RecordDuration);
        strapd(Buffer + DURATION, TempString, sizeof(EDFMainHeader.Duration));
        sprintf(TempString, "%u", Channels);
        /* Write the number of Channels */
        strapd(Buffer + NRCHANNELS, TempString, sizeof(EDFMainHeader.NrChannels));
        ...
        Size = sizeof(EDFChannelHeader.m_reserved);
        ...
        for (unsigned int i = 0; i < NrBlock; i++)
        {
            ...
            for (unsigned int j = 0; j < Channels; j++)
            {
                for (unsigned int k = 0; k < Data->RecordSamples[j]; k++)
                    FileBuffer[Index + k] = (int)((Buffer[j][i * Data->RecordSamples[j] + k] - Data->PhysicalMin[j]) / Ratio [j]) + DigitalMin[j];
                Index += Data->RecordSamples[j];
            }
            Data->NrRecords++;
            for (unsigned int i = 0; i < Channels; ++i)
                Data->m_total_samples[i] = Data->RecordSamples[i] * Data->NrRecords;
            if (!WriteFile(Data->FileHandle, FileBuffer, FILEBUFFER * sizeof(short int), &NrBytes, NULL))
                return false;
            if ((FILEBUFFER * sizeof(short int)) != NrBytes)
                return false;
        }
