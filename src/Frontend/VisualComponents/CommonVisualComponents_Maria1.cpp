...
void PlotPanel::DrawVCoord(HDC hdc, RECT* rect, double a_min_phy_s_x, double a_max_phy_s_x)
{
    ...
    */
    ...
    border[2] = 15;
    border[3] = 15;
    SelectObject(hdc, m_blue_brush); //D
    ...
    */
    {
        ...
        double values [(int)floor(physxxwidth / intlen) + 2]; /** < this will contain the numbers of the scale*/
        for (int i = 0; i < (int)floor(physxxwidth / intlen) + 2; i++) /** < same: i<values.size()*/
            if (((border[1] + floor(i * hossz1 / ints - rate1) + 1) > border[1]) && ((border[1] - 1 + floor(i * hossz1 / ints - rate1) < (ClientHeight - border[3]))))//ha az y menti skala teruleten vagyok
            {
        ...
        for (int i = 0; i < (int)floor(physxxwidth / intlen) + 2; i++) /** < iterate over values */
